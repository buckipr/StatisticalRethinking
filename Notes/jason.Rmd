...
* *Bayesian updating & posterior distribution:* We update our prior
information using the data, our likelihood, and multiplication.  The result is
a probability distribution for our model parameters, which we can use to make
inferences.
...
Consider a small globe that we toss up in the air (so that it spins) and when
we catch it we note if our right index finger is covering a portion of water
(W) or land (L).  Supposes we toss the globe 9 times and obtain the following
data: W, L, W, W, W, L, W, L, W.  Consider the proportion of the globe that is
covered with water, the parameter that we want to study.  If our prior belief
is that all of the proportions from 0 to 1 are equally likely, then the
following figure shows what happens as we toss the ball, observe on outcome,
and update our prior beliefs.
...
In the previous figure, we used an analytic solution to show the resul of
updating our beliefs.  For many problems, such an analytic solution does not
exist and we need to use other methods to perform inference on the parameters
of interest.  Three such methods are covered in this text

1. Grid approximation
1. Quadratic approximation
1. Monte carlo markov chain (MCMC)

Here we illustrate the grid and quadratic approximations using the globe
example.

```{r, echo = TRUE, include = TRUE}
p_grid <- seq(0, 1, length = 20)
#prior <- rep(1, 20)
prior <- rep(c(1,2,1), c(5,10,5))
likelihood <- dbinom(6, 9, prob = p_grid)
unstd_post <- likelihood * prior
posterior <- unstd_post / sum(unstd_post)
summary(posterior)
summary(dbeta(p_grid, 7, 4)/)
#plot(p_grid, posterior, type = 'b')
#posterior
#dbeta(p_grid, 7, 4)/ sum(dbeta(p_grid, 7, 4))
#lines(p_grid, dbeta(p_grid, 7, 4)/ sum(dbeta(p_grid, 7, 4)), col = 'red')
```
